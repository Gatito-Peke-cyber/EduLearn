/* =====================================================
   EduLearn — Inglés A1 — JS v1.6 — Fix duplicados v2
   CAMBIOS v1.6:
   - FIX DUPLICADOS CRÍTICO: syncXPToProfile ahora busca
     la inscripción existente por ID, cursoId, link Y nombre.
     Si la encuentra con un ID diferente (ej. 'p16' del catálogo),
     normaliza ese ID a COURSE.id ('ingles_a1') ANTES de hacer
     cualquier operación en Firestore, evitando que se creen
     dos documentos distintos para el mismo curso.
   - Se añade _removeDuplicatesFromLocal() que limpia entradas
     duplicadas del localStorage en el momento del sync.
   ===================================================== */
...
/* ─── FIX v1.6: _removeDuplicatesFromLocal ─────────────────────────────────
   Limpia el localStorage de entradas duplicadas del curso de inglés.
   Se llama una sola vez al sincronizar la nota del examen final.
   Detecta entradas duplicadas por link ('ingles.html') o por nombre
   que contenga 'inglés'/'ingles', y las fusiona en una sola entrada
   con el ID canónico COURSE.id ('ingles_a1').
   ───────────────────────────────────────────────────────────────────────── */
function _removeDuplicatesFromLocal(enrollments, canonicalId) {
  // Agrupar entradas que corresponden al mismo curso de inglés
  var inglesIdxs = [];
  enrollments.forEach(function(e, i) {
    var matchById   = e.id === canonicalId || e.cursoId === canonicalId;
    var matchByLink = e.link && (e.link === 'ingles.html' || e.link.includes('ingles'));
    var matchByName = e.nombre && (
      e.nombre.toLowerCase().includes('inglés') ||
      e.nombre.toLowerCase().includes('ingles')
    );
    if (matchById || matchByLink || matchByName) inglesIdxs.push(i);
  });

  if (inglesIdxs.length <= 1) return enrollments; // sin duplicados

  // Fusionar todas las entradas de inglés en una sola
  var merged = Object.assign({}, enrollments[inglesIdxs[0]]);
  merged.id      = canonicalId;
  merged.cursoId = canonicalId;

  for (var k = 1; k < inglesIdxs.length; k++) {
    var other = enrollments[inglesIdxs[k]];
    var prevNota  = typeof merged.nota_final === 'number' ? merged.nota_final : -1;
    var otherNota = typeof other.nota_final  === 'number' ? other.nota_final  : -1;
    if (otherNota > prevNota)          merged.nota_final = other.nota_final;
    if (other.aprobado === true)       merged.aprobado   = true;
    if (other.estado === 'completado') merged.estado     = 'completado';
  }

  // Reconstruir el arreglo sin las entradas duplicadas de inglés,
  // insertando la entrada fusionada en la posición del primero
  var result = [];
  enrollments.forEach(function(e, i) {
    if (inglesIdxs.indexOf(i) === -1) {
      result.push(e);       // no es inglés → conservar
    } else if (i === inglesIdxs[0]) {
      result.push(merged);  // primera aparición → insertar la fusionada
    }
    // el resto de duplicados de inglés → descartar
  });
  return result;
}
...
/* =====================================================
   syncXPToProfile — v1.6
   FIX DUPLICADOS CRÍTICO:
   1. Búsqueda ampliada: ID exacto → link → nombre.
   2. Si se encuentra con un ID distinto, se normaliza
      a COURSE.id ('ingles_a1') ANTES de operar en Firebase,
      evitando la creación de un segundo documento Firestore.
   3. _removeDuplicatesFromLocal() limpia el localStorage
      antes de guardarlo, eliminando entradas huérfanas.
   4. _deduplicateLocal() como capa final de seguridad.
   ===================================================== */
async function syncXPToProfile(courseXP, finalApproved, finalGrade, examType, examGrade) {
  try {
    ...
    /* ─── FIX v1.6: búsqueda ampliada ───────────────────────────────────
       Primero buscar por ID canónico (ingles_a1).
       Si no aparece, buscar por link 'ingles.html' (inscripciones del
       catálogo con id 'p16' u otro ID del catálogo).
       Si tampoco, buscar por nombre que contenga 'inglés' / 'ingles'.
       ─────────────────────────────────────────────────────────────────── */
    ...
    if (idx === -1) {
      idx = enrollments.findIndex(function(e) {
        return e.link && (e.link === 'ingles.html' || e.link.includes('ingles'));
      });
    }
    ...
    /* ─── FIX v1.6: normalizar ID al canónico ANTES de operar ──────────
       Si encontramos la entrada con un ID diferente (ej. 'p16'),
       actualizamos id y cursoId a COURSE.id en el objeto local.
       Así el enrollCourse de Firebase apuntará al mismo documento
       que ya existe o creará uno con el ID correcto (no duplicado).
       ─────────────────────────────────────────────────────────────────── */
    if (idx !== -1) {
      var existingId = enrollments[idx].cursoId || enrollments[idx].id;
      if (existingId !== COURSE.id) {
        console.log('[Ingles] Normalizando ID de inscripción:', existingId, '→', COURSE.id);
        enrollments[idx].id      = COURSE.id;
        enrollments[idx].cursoId = COURSE.id;
      }
    }

    if (idx === -1) {
      /* No existe ninguna inscripción para inglés → crearla */
      var newEntry = {
    ...
    /* ─── FIX v1.6: eliminar duplicados de inglés antes de guardar ───── */
    enrollments = _removeDuplicatesFromLocal(enrollments, COURSE.id);
    /* Capa extra de seguridad: deduplicar por ID/cursoId también */
    localStorage.setItem(enrollKey, JSON.stringify(_deduplicateLocal(enrollments)));

    /* Re-obtener el índice tras la deduplicación */
    idx = enrollments.findIndex(function(e) {
      return e.id === COURSE.id || e.cursoId === COURSE.id;
    });
    if (idx === -1) idx = 0; // fallback de seguridad
    ...
    try {
      ...
      /* PASO A: Crear/actualizar inscripción en Firestore con el ID canónico.
         Al usar COURSE.id como id, el docId siempre será {uid}_ingles_a1,
         evitando crear {uid}_p16 Y {uid}_ingles_a1 al mismo tiempo. */
      if (dbMod.enrollCourse) {
        await dbMod.enrollCourse(uid, Object.assign({}, enrollments[idx], { id: COURSE.id, cursoId: COURSE.id }));
      }
