/* =====================================================
   EduLearn — Primaria JS v5.0 — Firebase Sync
...
function queueOrSync(asyncFn) {
  if (currentUID) { asyncFn().catch(e => console.warn('[FB]', e)); }
  else            { _pendingQueue.push(asyncFn); }
}
async function _flushQueue() {
  while (_pendingQueue.length) { try { await _pendingQueue.shift()(); } catch(e){ console.warn('[FB flush]',e); } }
}
  ...
  for (const e of list) {
    try {
      await dbEnrollCourse(currentUID, {
        id:String(e.id), nombre:e.nombre||'', area:e.area||'', tipo:e.tipo||'permanente',
        horas:e.horas||0, img:e.img||'', link:e.link||'#', grado:e.grado||null,
        estado:e.estado||'en_progreso', inscritoEl:e.inscritoEl||new Date().toISOString(),
      });
...
function getTempStatus(temp) {
  const now=Date.now(), inicio=new Date(temp.inicio).getTime(), fin=new Date(temp.fin).getTime();
  if(now<inicio) return 'not_started'; if(now>fin) return 'expired'; return 'active';
}
function buildCountdown(targetIso) {
  const diff=new Date(targetIso).getTime()-Date.now(); if(diff<=0) return '00d 00h 00m 00s';
  const s=Math.floor(diff/1000), pad=n=>String(n).padStart(2,'0');
...
const GRADO_MAP = { '1':'1er Grado','2':'2do Grado','3':'3er Grado','4':'4to Grado','5':'5to Grado','6':'6to Grado' };
const PAGE_SIZE = 9;
...
function toast(msg, color='var(--green)'){
  ...
  t.textContent=msg; t.style.borderColor=color; t.style.color=color; t.classList.add('show');
  clearTimeout(t._tid); t._tid=setTimeout(()=>t.classList.remove('show'),2800);
}
function genStars(n){ return Array.from({length:5},(_,i)=>`<span class="${i<n?'star-filled':'star-empty'}">★</span>`).join(''); }
function getAreaLabel(a){
  const map={matematicas:'🔢 MATES',lectura:'📚 LECTURA',ciencias:'🔬 CIENCIAS',ingles:'🌍 INGLÉS',arte:'🎨 ARTE',musica:'🎵 MÚSICA',computacion:'💻 COMPU',valores:'💚 VALORES',historia:'🏛️ HISTORIA',ef:'⚽ EF'};
...
function enrollCourse(id){
  const c=CURSOS_P.find(x=>x.id===id); if(!c||isEnrolled(id)) return;
  const inscritoEl=new Date().toISOString();
  const list=getEnrolled();
  list.push({ tipo:'permanente',id:c.id,nombre:c.nombre,area:c.area,grado:c.grado,
    horas:c.horas,img:c.img,inscritoEl,estado:'en_progreso',link:c.link });
  lsSet(ENROLL_KEY,list);

  queueOrSync(()=>dbEnrollCourse(currentUID,{
    id:String(c.id),nombre:c.nombre,area:c.area||'',tipo:'permanente',
    horas:c.horas||0,img:c.img||'',link:c.link||'#',grado:c.grado||null,
    estado:'en_progreso',inscritoEl,
  }));
  addXP(c.xp);
  toast(`🎮 ¡INSCRITO EN: ${c.nombre}! +${c.xp} XP`);
  renderGrid();
  const mBtn=$('#modal-enroll-btn');
  if(mBtn&&mBtn.dataset.modalId===id){ mBtn.textContent='✓ INSCRITO'; mBtn.disabled=true; mBtn.classList.add('enrolled'); const ok=$('#modal-insc-ok'); if(ok) ok.style.display='flex'; }
}

function enrollTemporal(tid){
  const t=TEMPORALES_P.find(x=>x.id===tid); if(!t) return;
  ...
  const inscritoEl=new Date().toISOString();
  const list=getEnrolled();
  list.push({ tipo:'temporal',id:t.id,nombre:t.nombre,area:t.area,horas:t.horas,
    img:t.img,inscritoEl,estado:'en_progreso',inicio:t.inicio,fin:t.fin,link:t.link });
  lsSet(ENROLL_KEY,list);
  queueOrSync(()=>dbEnrollCourse(currentUID,{
    id:String(t.id),nombre:t.nombre,area:t.area||'',tipo:'temporal',
    horas:t.horas||0,img:t.img||'',link:t.link||'#',grado:null,
    estado:'en_progreso',inscritoEl,
  }));
  addXP(50);
...
function addXP(amount){
  const p=ls(PROFILE_KEY)||{}; p.xp=(p.xp||0)+amount; lsSet(PROFILE_KEY,p);
  queueOrSync(()=>dbAddXP(currentUID,amount));
}

/* ---- FILTER ---- */
function filteredCourses(){
  const favs=getFavs(); let list=[...CURSOS_P];
  if(state.favoritosOnly) list=list.filter(c=>favs.has(c.id));
  if(state.search) list=list.filter(c=>c.nombre.toLowerCase().includes(state.search.toLowerCase())||c.desc.toLowerCase().includes(state.search.toLowerCase()));
...
/* ---- RENDER ---- */
function renderGrid(){
  const list=filteredCourses(), pages=Math.max(1,Math.ceil(list.length/PAGE_SIZE));
  ...
  if(!slice.length){
    ...
    renderPagination(1,1); return;
  }
  const favs=getFavs();
  grid.innerHTML=slice.map(c=>{
    const enrolled=isEnrolled(c.id), fav=favs.has(c.id);
    return `
      ...
      <div class="card-body">
        ...
        <div class="card-foot">
          ...
          <div style="display:flex;gap:5px">
            ...
            ${enrolled?`<button class="btn btn-enroll enrolled" disabled>✓ INSCRITO</button>`:`<button class="btn btn-enroll" data-enroll-id="${c.id}">INSCRIBIRSE</button>`}
          </div>
        </div>
      </div>
    </div>`;
  ...
  requestAnimationFrame(()=>{ $$('#coursesGrid .reveal').forEach((el,i)=>setTimeout(()=>el.classList.add('visible'),i*55)); });
  $$('[data-fav-id]').forEach(btn=>btn.addEventListener('click',e=>{e.stopPropagation();toggleFav(btn.dataset.favId);}));
  $$('[data-view-id]').forEach(btn=>btn.addEventListener('click',e=>{e.stopPropagation();openModal(btn.dataset.viewId);}));
  $$('[data-enroll-id]').forEach(btn=>btn.addEventListener('click',e=>{e.stopPropagation();enrollCourse(btn.dataset.enrollId);}));
  $$('.card').forEach(card=>card.addEventListener('click',()=>openModal(card.dataset.id)));
}

function renderPagination(current,total){
  const prev=$('#prevPage'),next=$('#nextPage'),list=$('#pageList'); if(!prev) return;
  prev.disabled=current<=1; next.disabled=current>=total;
  list.innerHTML=Array.from({length:total},(_,i)=>`<li><button class="${i+1===current?'active':''}" data-pg="${i+1}">${i+1}</button></li>`).join('');
  list.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>{state.page=+b.dataset.pg;renderGrid();}));
}
...
function renderTemporales(){
  ...
  const grid=$('#tempGrid'); if(!grid) return;
  grid.innerHTML=TEMPORALES_P.map(t=>{
    const status=getTempStatus(t),enr=isEnrolled(t.id);
    const badgeTxt=status==='expired'?'⛔ EXPIRADO':status==='active'?'⚡ ACTIVO':'🔒 PRÓXIMO';
  ...
  $$('[data-enroll-temp]').forEach(btn=>btn.addEventListener('click',()=>enrollTemporal(btn.dataset.enrollTemp)));
}

/* ---- FAVORITES ---- */
function toggleFav(id){
  const favs=getFavs(); if(favs.has(id)) favs.delete(id); else favs.add(id);
  saveFavs(favs);
  const btn=$(`[data-fav-id="${id}"]`); if(btn) btn.classList.toggle('active',favs.has(id));
  if(state.favoritosOnly) renderGrid();
}

/* ---- MODAL ---- */
function openModal(id){
  const c=CURSOS_P.find(x=>x.id===id); if(!c) return;
  const enrolled=isEnrolled(id), modal=$('#courseModal'), modalBody=modal?.querySelector('.modal-body');
...
function closeModal(){ const modal=$('#courseModal'); if(!modal) return; modal.classList.remove('show'); modal.setAttribute('aria-hidden','true'); document.body.style.overflow=''; }

/* ---- CONTROLS ---- */
function initControls(){
  const search=$('#searchInput'),selArea=$('#areaFilter'),selGrado=$('#gradoFilter'),selSort=$('#ordenSelect'),btnReset=$('#btnReset'),btnFavs=$('#btnFavoritos');
  ...
  if(btnFavs)  btnFavs.addEventListener('click',()=>{state.favoritosOnly=!state.favoritosOnly;btnFavs.classList.toggle('active',state.favoritosOnly);state.page=1;renderGrid();});
  if(btnReset) btnReset.addEventListener('click',()=>{
    state={search:'',area:'',grado:'',sort:'relevancia',page:1,favoritosOnly:false};
    if(search)  search.value=''; if(selArea) selArea.value=''; if(selGrado) selGrado.value=''; if(selSort) selSort.value='relevancia'; if(btnFavs) btnFavs.classList.remove('active');
    renderGrid(); toast('🔄 FILTROS REINICIADOS');
  });
  $('#prevPage')?.addEventListener('click',()=>{if(state.page>1){state.page--;renderGrid();}});
  $('#nextPage')?.addEventListener('click',()=>{const f=filteredCourses(),p=Math.ceil(f.length/PAGE_SIZE);if(state.page<p){state.page++;renderGrid();}});
}

function initNav(){
  const ham=$('.hamburger'),nav=$('.nav'); if(ham&&nav) ham.addEventListener('click',()=>nav.classList.toggle('open'));
  const modal=$('#courseModal'),closeBtn=modal?.querySelector('.modal-close');
...
function initBackToTop(){ const btn=$('#backToTop'); if(!btn) return; window.addEventListener('scroll',()=>btn.classList.toggle('show',window.scrollY>400)); btn.addEventListener('click',()=>window.scrollTo({top:0,behavior:'smooth'})); }

function hideLoader(){
  const loader=$('#loader'); if(!loader) return;
  let w=0; const fill=loader.querySelector('.ld-fill');
  const iv=setInterval(()=>{
    w+=Math.random()*18+5; if(fill) fill.style.width=Math.min(w,100)+'%';
...
function generateEmojis(){
  const container=$('#floatingEmojis'); if(!container) return;
  const emojis=['🌟','🎮','🔢','📚','🎨','🎵','🔬','⚽','🌍','💻','🏆','❤️','🎯','🌈'];
  for(let i=0;i<18;i++){const el=document.createElement('span');el.className='float-emoji';el.textContent=emojis[Math.floor(Math.random()*emojis.length)];el.style.left=Math.random()*100+'%';el.style.animationDuration=(Math.random()*15+10)+'s';el.style.animationDelay=(Math.random()*10)+'s';container.appendChild(el);}
}

function generateStars(){
  const container=$('#floatingEmojis'); if(!container) return;
  const colors=['#FFE000','#39FF14','#00F8FF','#FF80EE','#ffffff'];
  for(let i=0;i<60;i++){const s=document.createElement('span');const size=Math.random()*2+1;s.style.cssText=`position:absolute;left:${Math.random()*100}%;top:${Math.random()*100}%;width:${size}px;height:${size}px;background:${colors[Math.floor(Math.random()*colors.length)]};opacity:${Math.random()*0.35+0.05};animation:blink ${Math.random()*3+2}s step-end infinite;animation-delay:${Math.random()*3}s;pointer-events:none;`;container.appendChild(s);}
}

function initReveal(){ const obs=new IntersectionObserver((entries)=>{entries.forEach(e=>{if(e.isIntersecting)e.target.classList.add('visible');});},{threshold:0.08}); $$('.reveal').forEach(el=>obs.observe(el)); }

/* ---- INIT ---- */
document.addEventListener('DOMContentLoaded', ()=>{
  hideLoader(); generateStars(); generateEmojis();
  initNav(); initControls(); renderGrid(); renderTemporales(); initBackToTop(); initReveal();
});
...
onAuthChange(async (user) => {
  ...
  if (currentUID) {
    await _flushQueue();
    await _syncAllLocalEnrollments();
  }
});